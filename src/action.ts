...
const SUPPORTED_DOMAINS = ["devops", "security", "performance"] as const;
...
async function main() {
  ...
  if (!SUPPORTED_DOMAINS.includes(domain as typeof SUPPORTED_DOMAINS[number])) {
    console.warn(
      `Unsupported domain '${domain}', falling back to 'devops'. Supported domains: ${SUPPORTED_DOMAINS.join(", ")}`
    );
    domain = "devops";
  }
  ...
  try {
    ...
    console.error("agent-evals action failed:", (error as Error).message);
    process.exit(1);
  }
}
