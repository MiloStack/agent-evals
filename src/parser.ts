...
import type { EvalInput, FileEntry } from "./types.js";
...
function collectFiles(dir: string): FileEntry[] {
  ...
  for (const item of readdirSync(dir)) {
    ...
    if (stat.isDirectory()) entries.push(...collectFiles(full));
    else if (stat.isFile()) {
      try { entries.push({ path: full, content: readFileSync(full, "utf-8"), language: detectLanguage(full) }); }
...
export function parseInput(target: string): EvalInput {
  const stat = statSync(target);
  if (stat.isFile()) {
    const content = readFileSync(target, "utf-8");
    return { files: [{ path: target, content, language: detectLanguage(target) }] };
  }
  if (stat.isDirectory()) return { files: collectFiles(target) };
