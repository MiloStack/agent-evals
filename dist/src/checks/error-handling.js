...
export const errorHandling = {
    ...
    async run(input) {
        ...
        if (!code.length)
            return { check: "error-handling", passed: true, score: 100, details: "No code files.", findings: [] };
        for (const file of code) {
            ...
            if (file.language === "python") {
                ...
                for (let i = 0; i < lines.length; i++) {
                    BARE_EXCEPT.lastIndex = 0;
                    if (BARE_EXCEPT.test(lines[i]))
                        findings.push({ file: file.path, line: i + 1, message: "Bare except: pass.", severity: "error" });
                }
            }
        }
